...
void tempset::paintEvent(QPaintEvent *event)
{
...
void tempset::setOtherBackgroud(bool status,QPainter *painter)
{
    if(status == true)
    {
        painter->save();
...
//绘制控件
void tempset::drawDashBoardWiget(QPainter *painter)
{
    //全局参数初始化
    qParmInit();
...
//绘制仪表盘
void tempset::drawDashBoard(QPainter *painter)
{
    painter->save();
    ...
    painter->setBrush(qBrush);
    painter->drawEllipse(qCircular,20,20);//内部最大粉色圆盘


    painter->restore();
}

void tempset::drawDivideRule(QPainter *painter)
{
    painter->save();
...
void tempset::drawDashBoardText(QPainter *painter)
{
    painter->save();
    ...
    //绘制表盘文字
    QFont qFont("楷体",18,QFont::Bold,false);
    painter->setFont(qFont);
    if (none) painter->drawText(QRectF(qCircular.x() - 17,qCircular.y() + 65,80,80),QString::number(qValue-20));
    else painter->drawText(QRectF(qCircular.x() - 17,qCircular.y() + 65,80,80),"N/A");
    float fRadius = 0.0;
    //通过公式计算刻度文字的位置信息。
    for(int uiLoop = 0;uiLoop < (fMaxDivide ) + 1 ;uiLoop++)
    {
        ...
        fRadius = (qRadius - 60 - 18);
        fAngle += 4;


        int x1Start = qCircular.x() + fRadius * cos(fAngle * 3.14 / 180)-7;
        int y1Start = qCircular.y() + fRadius * sin(fAngle * 3.14 / 180)-7;

        QFont qFonts("楷体",8,QFont::Bold,false);
        painter->setFont(qFonts);

        painter->drawText(QRectF(x1Start,y1Start,80,80),QString::number((fMaxValue / fMaxDivide )* uiLoop-20));
    }

    painter->restore();
}

void tempset::drawDishBoardPointer(QPainter *painter)
{
    painter->save();
...
void tempset::setValue(float qValue)
{
    this->qValue = qValue+20;
    this->none=true;
}

void tempset::setValuenone()
{
    this->qValue = 0;
    this->none=false;
}

void tempset::setMaxValue(float fMaxValue)
{
    this->fMaxValue = fMaxValue;
}

void tempset::setDashBoardEdgeColor(QColor &qColor)
{
    qDialEdgeColor = qColor;
}

void tempset::setDashDivideBackgroudColor(QColor &qColor)
{
