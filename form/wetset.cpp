...
void wetset::paintEvent(QPaintEvent *event)
{
...
void wetset::setOtherBackgroud(bool status,QPainter *painter)
{
    if(status == true)
    {
        painter->save();
...
//绘制控件
void wetset::drawDashBoardWiget(QPainter *painter)
{
    //全局参数初始化
    qParmInit();
...
//绘制仪表盘
void wetset::drawDashBoard(QPainter *painter)
{
    painter->save();
...
void wetset::drawDivideRule(QPainter *painter)
{
    painter->save();
...
void wetset::drawDashBoardText(QPainter *painter)
{
    painter->save();
    ...
    if (none) painter->drawText(QRectF(qCircular.x() - 17,qCircular.y() + 65,80,80),QString::number(qValue));
...
void wetset::drawDishBoardPointer(QPainter *painter)
{
    painter->save();
...
void wetset::setValue(float qValue)
{
    this->qValue = qValue;
    this->none=true;
}

void wetset::setValuenone()
{
    this->qValue = 0;
    this->none=false;
}

void wetset::setMaxValue(float fMaxValue)
{
    this->fMaxValue = fMaxValue;
}

void wetset::setDashBoardEdgeColor(QColor &qColor)
{
    qDialEdgeColor = qColor;
}

void wetset::setDashDivideBackgroudColor(QColor &qColor)
{
