...
SendEmailThread::SendEmailThread(QObject *parent) : QThread(parent)
{
    stopped = false;
    emialTitle = "警报";
    sendEmailAddr = "m18814474582@163.com";
    sendEmailPwd = "GYJSGJZBKNJBQCWM";
    receiveEmailAddr = "2281002841@qq.com";
    contents.clear();
    fileNames.clear();
}
...
void SendEmailThread::run()
{
    while (!stopped) {
        ...
        if (count > 0) {
            ...
            SmtpClient smtp("smtp.163.com", tempPort, tempPort == 25 ? SmtpClient::TcpConnection : SmtpClient::SslConnection);
...
void SendEmailThread::append(const QString &content)
{
    mutex.lock();
